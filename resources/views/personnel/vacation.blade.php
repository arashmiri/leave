                        ...
                        <div class="flex items-center space-x-6 space-x-reverse rounded-xl shadow-lg p-3 px-6 bg-white w-full">
                            <div class="flex flex-wrap space-x-10 space-y-3 px-10">
                                <div class="dateShow flex items-center space-x-10 space-x-reverse">
                                    <span class="font-bold"> {{$personnel->rank}}  {{$personnel->name}}</span>
                                    <span>شماره کارگزینی: {{$personnel->personnel_code}}</span>                                    
                                    <span>تاریخ رفت: {{$vacation->start}}</span>
                                    <span id="endShow">تاریخ برگشت: {{$vacation->end}}</span>
                                    <span>آخرین حضور: {{$vacation->attendance}}</span>
                                </div>
                                <div class="flex items-center space-x-10 space-x-reverse">
                                    <span>استحقاق استفاده شده:  {{$vacation->entitlement}} روز</span>
                                    <span>تشویقی: {{$vacation->encouragement}} </span>
                                    <span>علت تشویقی:  {{$vacation->encouragementDescription}}</span>
                                    <!-- <span>مسافت:  {{$personnel->distance}} روز</span> -->
                                    <!-- <span>جمع کل روزها:  {{$vacation->entitlement  + $vacation->encouragement + $vacation->distance }}</span> -->
                                </div>
                            </div>
                        </div>
